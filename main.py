...
def get_subjects_by_date(
    ...
    """
    Return EFFECTIVE subjects for a faculty on a particular date.

    ============================================================
    NORMAL
    ============================================================

    Faculty gets normal subjects from:
    ...
    If the logged-in faculty is the original faculty and
    that faculty is on leave for a particular period/section,
    that normal period is removed.

    ============================================================
    SUBSTITUTE
    ============================================================

    If the logged-in faculty is the substitute_faculty_id
    for a faculty_leave_substitute record, the ORIGINAL
    faculty's subject/period/section is added.

    IMPORTANT:

        The substitute teaches the ORIGINAL class.
    ...
    are returned for the substitute attendance entry.

    substitute_subject_id is NOT used to replace the
    original subject.
    ...
    faculty_leave_substitute acts only as an
    effective timetable override layer.
    ...
    A faculty can have:
        ...
        Physics P2  -> NORMAL
        Physics P3  -> SUBSTITUTE

    Even though both have the same subject_id,

        PHY1

    they MUST remain two separate attendance entries.

    Therefore the logical identity includes:
    ...
    try:
        ...
        # ============================================================
        # 4. GET NORMAL SUBJECTS
        #
        # This query is kept for compatibility/debugging.
        # The actual period-level records are loaded below.
        # ============================================================
        ...
        # ============================================================
        # 5. GET NORMAL SUBJECTS AT PERIOD LEVEL
        #
        # IMPORTANT:
        #
        # Leave/substitution is period + section based.
        #
        # Therefore we must NOT use only a subject-level query.
        # ============================================================

        normal_periods = []

        weekday_short = parsed_date.strftime(
            "%a"
        ).strip()
        ...
        normal_db_rows = cur.fetchall()

        for r in normal_db_rows:
        ...
        #
        # We retrieve records where the logged-in faculty is either:
        #
        #   original_faculty_id
        #
        # OR
        ...
        cur.execute(
            ...
            WHERE LOWER(TRIM(department))
              ...
              AND (
                    LOWER(TRIM(original_faculty_id))
                        = LOWER(TRIM(%s))

                    OR

                    LOWER(TRIM(substitute_faculty_id))
                        = LOWER(TRIM(%s))
              )
        ...
        # Leave:
        #
        #     original_faculty_id = MATH01
        #     period_no = 3
        #
        # Result for Math faculty:
        #
        #     Mathematics P3 is removed.
        #
        # ============================================================
        ...
        for item in normal_periods:
            ...
            for lr in leave_rows:
                ...
                # ----------------------------------------------------
                # This leave record belongs to another faculty.
                # ----------------------------------------------------

                if original_faculty_id.lower() != faculty_id.lower():
                    continue
                ...
                if leave_subject_id.lower() != item_subject_id.lower():
                    continue

                # ----------------------------------------------------
                # SECTION
                #
                # ALL means every section.
                # Otherwise exact section match.
                # ----------------------------------------------------

                if (
                    leave_section.upper() != "ALL"
                    and leave_section.upper() != item_section.upper()
                ):
                    continue

                # ----------------------------------------------------
                # THIS NORMAL PERIOD IS ON LEAVE
                # ----------------------------------------------------

                is_on_leave = True

                print(
                    "🚫 REMOVING LEAVE PERIOD:",
                    item["subject_name"],
                    "| Subject:",
                    item_subject_id,
                    "| Period:",
            ...
            # --------------------------------------------------------
            # Keep only periods for which faculty is NOT on leave.
            # --------------------------------------------------------

            if not is_on_leave:

                filtered_normal_periods.append(
                    item
                )

        print(
            "🚫 NORMAL PERIODS AFTER LEAVE FILTER:",
            len(filtered_normal_periods)
        )
        ...
        # CRITICAL RULE:
        #
        # The substitute faculty teaches the ORIGINAL faculty's class.
        #
        # Therefore we ALWAYS use:
        #
        #     lr[5] = original subject_id
        #
        # and NOT:
        #
        #     lr[8] = substitute_subject_id
        #
        # Even if substitute_subject_id exists.
        #
        # Example:
        #
        # Math faculty:
        ...
        # Leave/substitute:
        #
        #     Math → P3 → Physics faculty
        #
        # Physics result:
        #
        #     Physics P2 NORMAL
        #     Mathematics P3 SUBSTITUTE
        #
        # Flutter:
        #
        #     Physics (PHY1)
        ...
        for lr in leave_rows:
            ...
            # --------------------------------------------------------
            # Only the assigned substitute gets this entry.
            # --------------------------------------------------------

            if substitute_faculty_id.lower() != faculty_id.lower():
                continue
            ...
            # ORIGINAL SUBJECT
            #
            # This is the subject the substitute is actually teaching.
            # --------------------------------------------------------
            ...
            if not original_subject_id:
                print(
                    "⚠️ SUBSTITUTE RECORD SKIPPED:"
                    " original subject_id is empty"
                )
                continue

            # --------------------------------------------------------
            # DO NOT use substitute_subject_id here.
            #
            # substitute_subject_id is NOT the attendance subject
            # for this adjusted period.
            # --------------------------------------------------------
        ...
        # ============================================================
        # 9. COMBINE NORMAL + SUBSTITUTE
        #
        # A substitute faculty may have:
        #
        #     NORMAL class
        #
        # and
        ...
        print(
            "📚 TOTAL EFFECTIVE PERIOD RECORDS:",
            len(effective_periods)
        )

        # ============================================================
        # 10. REMOVE TRUE DUPLICATES
        #
        # IMPORTANT:
        #
        # NORMAL and SUBSTITUTE are NOT duplicates.
        #
        # Therefore is_substitute is part of the key.
        #
        # Example:
        #
        #     PHY1 / P2 / A / False
        ...
        #
        # Even if the period happens to be the same:
        #
        #     PHY1 / P3 / A / False
        #
        #     PHY1 / P3 / A / True
        #
        # they are still separate attendance makers.
        # ============================================================
        ...
        for item in effective_periods:
            ...
            # --------------------------------------------------------
            # NORMAL and SUBSTITUTE must have different identities.
            # --------------------------------------------------------
            ...
            if key not in unique_periods:
                ...
                print(
                    "⚠️ DUPLICATE EFFECTIVE PERIOD IGNORED:",
                    key
                )
        ...
        # 11. SORT BY PERIOD
        #
        # If same period contains:
        #
        #     NORMAL
        #     SUBSTITUTE
        #
        # NORMAL is shown first.
        # ============================================================
        ...
        # CRITICAL:
        #
        # Do NOT group only by subject_id.
        #
        # Group by:
        #
        #     subject_id
        ...
        #     Physics SUBSTITUTE
        #
        # remain two separate attendance makers.
        # ============================================================
        ...
        for item in effective_periods:
            ...
            if (
                ...
                or (
                    item_period is not None
                    and (
                        current_first is None
                        or item_period < current_first
                    )
                )
            ):
        ...
        # ============================================================
        # 13. SORT FINAL SUBJECTS BY FIRST PERIOD
        # ============================================================
        ...
        print()
        print("=" * 80)
        print("📚 EFFECTIVE FACULTY SUBJECTS")
        print("=" * 80)
...
def _verify_assignment_subject_assignment(
    cur,
    faculty_id: str,
    department: str,
    semester: str,
    ...
    """
    Assignment Marks are theory-only and have no section selector.

    Faculty assignment is resolved from timetable_slots, which is the
    authoritative faculty workload source.

    The selected subject must:
        - belong to the selected faculty
        - belong to the selected department
        - belong to the selected semester
        - be a theory subject
        - not be a practical/lab/workshop subject
    """
    ...
    if cur.fetchone() is None:
        raise HTTPException(
            status_code=403,
            detail=(
                "This theory subject is not assigned to the "
                "faculty for the selected class."
            ),
        )


# ======================================================
# GET ASSIGNMENT THEORY SUBJECTS
# ======================================================

@app.get("/assignment-subjects")
def get_assignment_subjects(
    ...
    """
    Return theory subjects assigned to the faculty for:

        Faculty + Department + Semester

    Assignment Marks do NOT use a section selector.
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Failed to load assignment subjects: "
                f"{str(e)}"
            ),
        )
...
def get_assignment_marks(
    ...
    try:
        ...
        _verify_assignment_subject_assignment(
            cur,
            faculty_id,
            department,
            semester,
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Failed to load assignment marks: "
                f"{str(e)}"
            ),
        )
...
@app.post("/assignment-marks")
def save_assignment_marks(
    payload: dict = Body(...)
):
    """
    Save Assignment marks for the complete active class.
    ...
    try:
        ...
        for index, item in enumerate(
            ...
            if (
                ...
                raise HTTPException(
                    status_code=400,
                    detail=(
                        f"Student {sbrn} is not an active "
                        "student of the selected department "
                        "and semester."
                    ),
                )
            ...
            if (
                ...
                if (
                    numeric_marks < 0
                    or numeric_marks > max_marks
                ):

                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Marks for {sbrn} must be "
                            f"between 0 and {max_marks}."
                        ),
                    )
            ...
            # --------------------------------------------------
                ...
                # Serialize ID generation so two simultaneous
                # faculty saves cannot generate the same ID.
                ...
                # Keep PostgreSQL sequence synchronized when
                # exam_marks.id uses a sequence.
                ...
                try:
                    ...
                    print(
                        "⚠️ ASSIGNMENT sequence sync skipped:",
                        sequence_error,
                    )
        ...
        try:
            ...
            print(
                "⚠️ ASSIGNMENT broadcast skipped:",
                broadcast_error,
            )

        print(
            "💾 ASSIGNMENT MARKS SAVED → "
            f"Faculty={faculty_id} | "
            f"Department={department} | "
            f"Semester={semester} | "
            f"Subject={subject_id} | "
...
# ======================================================
#
# House Test follows the same rules as Class Tests:
#   * theory subjects only
#   * faculty assignment from timetable_slots
#   * no section filter
#   * all active students in department + semester
#   * exam_marks logical identity:
#       sbrn + semester + exam_type + subject_id
#
# The saved exam_type is always exactly "House Test".
# ======================================================

@app.get("/house-test-subjects")
def get_house_test_subjects(
    faculty_id: str,
    department: str,
    semester: str,
):
    ...
    try:
        ...
        print("❌ HOUSE TEST SUBJECTS ERROR:", e)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load house test subjects: {str(e)}",
        )
    finally:
        release_db(conn)


@app.get("/house-test-marks")
def get_house_test_marks(
    faculty_id: str,
    department: str,
    semester: str,
    subject_id: str,
    ...
    """
    Load saved House Test marks.

    House Test has one fixed exam type:
        House Test
    ...
    try:
        ...
        cur.execute("""
            ...
            LEFT JOIN exam_marks em
             ...
             AND LOWER(TRIM(COALESCE(em.subject_id, '')))
                   = LOWER(TRIM(%s))
            WHERE LOWER(TRIM(COALESCE(st.department, '')))
                    = LOWER(TRIM(%s))
              AND LOWER(TRIM(COALESCE(st.semester, '')))
                    = LOWER(TRIM(%s))
              AND COALESCE(st.status_locked, 0) = 0
        ...
        print("❌ HOUSE TEST MARKS GET ERROR:", e)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load house test marks: {str(e)}",
        )
    finally:
        release_db(conn)


@app.post("/house-test-marks")
def save_house_test_marks(payload: dict = Body(...)):
    """
    Save House Test marks for the complete active class.

    Blank/null marks clear the saved value.
    There is intentionally NO section field.
    The fixed exam_type is "House Test".
    """
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail="Invalid payload")
    ...
    try:
        ...
        for index, item in enumerate(students_payload, start=1):
            ...
            if numeric_marks is None:
                if existing is not None:
                    cur.execute(
                        "DELETE FROM exam_marks WHERE id = %s",
                        (existing[0],),
                    )
                    cleared += 1
                continue
            ...
            if existing is not None:
                ...
                cur.execute("SELECT pg_advisory_xact_lock(7465321)")
                ...
                try:
                    ...
                    print(
                        "⚠️ HOUSE TEST sequence sync skipped:",
                        sequence_error,
                    )
        ...
        try:
            ...
            print(
                "⚠️ HOUSE TEST broadcast skipped:",
                broadcast_error,
            )

        print(
            "💾 HOUSE TEST MARKS SAVED → "
            f"Faculty={faculty_id} | "
            f"Department={department} | "
            f"Semester={semester} | "
            f"Subject={subject_id} | "
        ...
        return {
            "status": "success",
            "message": "House Test marks saved successfully",
            "faculty_id": faculty_id,
            "department": department,
            "semester": semester,
            "subject_id": subject_id,
        ...
        conn.rollback()
        print("❌ HOUSE TEST MARKS SAVE ERROR:", e)
        raise HTTPException(
            status_code=500,
            detail=f"Failed to save house test marks: {str(e)}",
        )
    finally:
        release_db(conn)
...
@app.get("/workshop/attendance-exists")
def workshop_attendance_exists(
    department: str,
    semester: str,
    group: str,
    date: str
